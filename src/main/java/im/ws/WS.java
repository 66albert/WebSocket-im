...
import im.config.CustomSpringConfigurator;
...
public class WS {
	...
	public void onOpen(Session session,@PathParam("userId") Integer userId) {
		...
		logger.info("用户"+userId+"进入ws,当前在线人数为" + mapUS.size() );
    ...
    public void onClose(Session session) {
    	...
    	if(userId!=null&&userId!=""){
			...
			logger.info("用户"+userId+"退出ws,当前在线人数为" + mapUS.size());
    	}
    }
...
//    			  			 mapUS.get(toId+"").getAsyncRemote().sendText(toMessage.toString());               //发送消息给对方
    		   		...
    		   		case "group":
						...
						/***查询群备注名称end，如果聊天框内不需要显示备注（只显示用户昵称）可以注释掉这段毕竟每次发送查询耗时***/
    		   				JSONArray memberList=JSONArray.fromObject(qun.getSimpleMemberByGroupId(toId));  //获取群成员userId列表
