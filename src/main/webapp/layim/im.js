...
layui.use('layim', function(layim){
 ...
 layim.config({
    ...
    ,min:false
    ,title: nickName        //主面板最小化后显示的名称
    ,msgbox: layui.cache.dir + 'css/modules/layim/html/msgbox.html' //消息盒子页面地址，若不开启，剔除该项即可
    //,find: layui.cache.dir + 'css/modules/layim/html/find.html' //发现页面地址，若不开启，剔除该项即可
    ,find: '/friend/searchFriendPage' //发现页面地址，若不开启，剔除该项即可
  ...
  layim.on('ready',function(){
      ...
      //获取消息盒子，消息数量
      $.post("/message/systemMsgNum?userId="+userId,function(res){
          console.log(res);
          if(res.num > 0){
              layim.msgbox(res.num);
          }
  ...
  layim.on('sign', function(value){
	  ...
	  if(value.length<200){
		  $.post("/index/updateUserSign",{"userId":userId,"sign":value},function(result){
			  console.log(result);
		  })
	  }else{
